...
class News_manage_Model extends Model {
	...
	$this->db->insert("news", array('title' => $title,
            'description' =>$description,'tstamp' => $t,'likes' => $likes,'linkedin' => $linkedin
    ...
    public function upload_image($file, $type) {

        

        $this->db->insert("news_image", array('file' => $img));
...

    {
       ...
       return $this->db->select("select *,news.id as newss_id from news join news_image ON news.id=news_image.nid  group by news.id desc");

    }
	
	
	
    public function getnewsimgss()

    {

       return $this->db->select("select * from news_image ");
    ...
    public function getupdatenews($data)
    {
          ...
          $Valres['description'] = $data['description'];
           $Valres['likes'] = $data['likes'];
            $Valres['linkedin'] = $data['linkedin'];
